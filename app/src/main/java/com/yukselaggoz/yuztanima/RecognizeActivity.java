...
import static com.yukselaggoz.yuztanima.TrainHelper.ACCEPT_LEVEL;

public class RecognizeActivity extends Activity implements CvCameraPreview.CvCameraViewListener {
    public static final String TAG = "RecognizeActivity";

    private CascadeClassifier faceDetector;
    private String[] nomes = {"", "Tahmin (4 uzrnden):"};
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        new AsyncTask<Void,Void,Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    faceDetector = TrainHelper.loadClassifierCascade(RecognizeActivity.this, R.raw.frontalface);
                ...
                findViewById(R.id.btReset).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        try {
                            ...
                            Toast.makeText(getBaseContext(), "Sifirlama Basarili", Toast.LENGTH_SHORT).show();
                            finish();
                        }catch (Exception e) {
                            Log.d(TAG, e.getLocalizedMessage(), e);
                        }
    ...
    void train() {
        ...
        if(remainigPhotos > 0) {
            Toast.makeText(getBaseContext(), "Kalan Foto: "+ remainigPhotos, Toast.LENGTH_SHORT).show();
            return;
        }else if(TrainHelper.isTrained(getBaseContext())) {
            Toast.makeText(getBaseContext(), "Egitildi", Toast.LENGTH_SHORT).show();
            return;
        }

        Toast.makeText(getBaseContext(), "Egitimi Baslat: ", Toast.LENGTH_SHORT).show();
        new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPostExecute(Void aVoid) {
                super.onPostExecute(aVoid);
                try {
                    Toast.makeText(getBaseContext(), "Egitim Tamamlandi : "+ TrainHelper.isTrained(getBaseContext()), Toast.LENGTH_SHORT).show();
                    finish();
                }catch (Exception e) {
                    Log.d(TAG, e.getLocalizedMessage(), e);
                }
    ...
    void noTrainedLabel(opencv_core.Rect face, Mat rgbaMat) {
        ...
        putText(rgbaMat, "Egitim Baslatilmadi", new Point(x, y), FONT_HERSHEY_PLAIN, 1.4, new opencv_core.Scalar(0,255,0,0));
    }
